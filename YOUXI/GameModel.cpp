...
void GameModel::startGame(GameType type)
{
    ...
    {
        std::vector<int> lineBoard; // verctor<int>是声明向量容器lineBoard
        for (int j = 0; j < kBoardSizeNum; j++)
            lineBoard.push_back(0);//在容器的最后添加一个值为0的数据，相当于 lineBoard[j]=0
        gameMapVec.push_back(lineBoard); //在容器的最后添加一个值为lineBoard的数据,相当于lineBoard[i][j]=0;
    }
